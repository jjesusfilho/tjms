ler_dados_cpopg_tjms <- function (arquivos = NULL, diretorio = ".", wide = FALSE) 
{
  ...
  dados <- purrr::map_dfr(arquivos, purrr::possibly(purrrogress::with_progress(~{
    ...
    codigo <- resposta %>% xml2::xml_find_all("//a[contains(@href,'processo.codigo')]") %>% 
      xml2::xml_attr("href") %>% stringr::str_extract("(?<=processo.codigo=)\\w+")
    
    if (length(codigo) > 1) {
      codigo <- duplicated(codigo) %>% which() %>% codigo[.] %>% 
        unique()
    }
    ...
    valor <- resposta %>% xml2::xml_find_all("//table[@id=''][@class='secaoFormBody']//*[@width='150']/following-sibling::td") %>% 
      xml2::xml_text() %>% stringr::str_squish()
    tibble::tibble(processo = processo, codigo_processo = codigo, 
